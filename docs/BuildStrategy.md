...
前提として、ホストには **Docker と uv** が必要です。また、ワークスペースをクローンした直後に `uv sync` を実行し、依存関係を同期してから開発を開始します。
...
1. ワークスペースをクローン
2. `uv sync` を実行して依存関係を同期
3. 初回ビルドで `compile_commands.json` を生成

この順序を守ることで、DevContainer の IntelliSense と CMake 構成が安定します。
...
ビルドは実行環境に応じてスクリプトを使い分けます。

- **ホスト OS から実行**: `scripts/build.sh`
- **コンテナ内部で実行**: `scripts/build_internal.sh`

ホスト側の標準手順は `scripts/build.sh` を実行する方法です。
...
`scripts/build_internal.sh` は DevContainer / ビルドコンテナ内部向けで、同様に `uv sync` 後に CMake ビルドを実行します。
...
- **スクリプトの実行場所**: `scripts/build.sh` はホスト専用、`scripts/build_internal.sh` はコンテナ専用です。実行環境とスクリプトを必ず対応させてください。
