...
auto tryRelaxNeighbor(const types::MapData &map, const StartGoalInfo &startGoal,
...
[[nodiscard]] auto computePrevious(const types::MapData &map, const StartGoalInfo &startGoal)
  ...
  while (!frontier.empty()) {
    ...
    for (const auto &move : moves) {
      tryRelaxNeighbor(map, startGoal, currentNode, move, distances, previous, frontier);
    }
  }
